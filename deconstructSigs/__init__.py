...
class DeconstructSigs(object):
    ...
    def __init__(self, maf,
        ...
        :param maf: The path to a folder filled with multiple *.maf files or a maf file path to be used in the analysis
        ...
        handler.setFormatter(logging.Formatter(Settings.log_format, Settings.log_time))
        ...
        self.analysis_handle  = analysis_handle or os.path.basename(self.maf).split('.')[0]

        #self.hg19_fasta_path = hg19_fasta_path
        self.hg19_fasta = Fasta(hg19_fasta_path) if hg19_fasta_path else None
        ...
        self._setup_subs_dict()
        self._load_cosmic_signatures()
        self._load_cosmic_signature_explanations()
        ...
        if context_counts is None:
            ...
            self._add_context_counts_to_subs_dict(context_counts)
    ...
    def figures(self, weights, explanations=False):
        ...
        reconstructed_profile = self._plot_reconstructed_profile(weights, y_max=y_max)
        if self.output_folder:
        ...
        self._plot_difference(reconstructed_profile, y_max)
        if self.output_folder:
    ...
    def _add_context_counts_to_subs_dict(self, context_counts):
    ...
    def _calculate_ignorable_signatures(self):
        """
    ...
    def _which_signatures(self, signatures_limit=None, associated=None):
        ...
        ignorable_signatures = self._calculate_ignorable_signatures()
        self._status('Signatures ignored because of outlying contexts:')
        for s in ignorable_signatures:
            self._status('- {} ignored because of outlying context {} with fraction {}'
                          .format(s.get('name'),
        ...
        while error_diff > error_threshold:
            ...
            error_pre = self._get_error(T, self.S, w)
            if error_pre == 0:
                break
            self._status("Iter {}:".format(iteration))
            self._status("- Pre error : {}".format(error_pre))
            w = self._update_weights(T, self.S, w,
                                      signatures_limit=signatures_limit,
                                      ignorable_signature_indices=ignorable_indices)
            error_post = self._get_error(T, self.S, w)
            self._status("- Post error: {}".format(error_post))
            error_diff = (error_pre - error_post) / error_pre
            self._status("- New normalized weights: ")
            self._print_normalized_weights(w)

        normalized_weights = w/sum(w)

        # Filter out any weights less than 0.6
        np.place(normalized_weights, normalized_weights < Settings.sig_cutoff, 0)
        return normalized_weights

    def _print_normalized_weights(self, w):
        """A standard way to print normalized weights given a vector of potentially not yet normalized weights"""
        normalized_weights = w / sum(w)
        for i, weight in enumerate(normalized_weights):
            if weight != 0:
                self._status("  {}: {}".format(self.signature_names[i], weight))
    ...
    def plot_sample_profile(self):
        ...
        _, flat_bins, flat_counts = self._get_plottable_flat_bins_and_counts()
        total_counts = sum(flat_counts)
        fractions = [float(c)/total_counts for c in flat_counts]
        y_max = max(fractions) * 1.05

        title = 'Tumor Profile'
        if self.analysis_handle:
            title = '{} for {}'.format(title, self.analysis_handle)
        self._plot_counts(flat_bins, fractions, y_max=y_max, title=title)
        return y_max

    def _plot_difference(self, reconstructed_tumor_weights, y_max):
        ...
        self._plot_counts(flat_bins, differences,
                           y_max=y_max, y_min=-y_max, title=title)

    def _plot_reconstructed_profile(self, weights, y_max=1):
        """Given a set of weights for each signature plot the reconstructed tumor profile using the cosmic signatures"""
        reconstructed_tumor_profile = self._get_reconstructed_tumor_profile(self.S, weights)
        ...
        alpha_flat_subs, _, alpha_flat_counts = self._get_alphabetical_flat_bins_and_counts()
        ...
        flat_subs, flat_bins, _ = self._get_plottable_flat_bins_and_counts()
        ...
        if self.analysis_handle:
            title = '{} for {}'.format(title, self.analysis_handle)
        self._plot_counts(flat_bins, reconstructed_tumor_counts, y_max=y_max, title=title)
        return reconstructed_tumor_counts

    def _plot_counts(self, flat_bins, flat_counts, title='Figure', y_max=1.0, y_min=0.0):
        ...
        for ax, data, color in zip(axes, [1, 2, 3, 4, 5, 6], colors):
            ...
            plt.setp(ax.xaxis.get_majorticklabels(), rotation=90, font_properties=FontProperties(family=Settings.font_family, weight=Settings.font_weight), color='k')
    ...
    def _get_plottable_flat_bins_and_counts(self):
    ...
    def _get_alphabetical_flat_bins_and_counts(self):
    ...
    def _setup_subs_dict(self):
        ...
        for sub in [self._standardize_subs('C', 'A'),
                    self._standardize_subs('C', 'G'),
                    self._standardize_subs('C', 'T'),
                    self._standardize_subs('T', 'A'),
                    self._standardize_subs('T', 'C'),
                    self._standardize_subs('T', 'G')]:
            ref = sub[0]
    ...
    def _load_cosmic_signatures(self):
    ...
    def _load_cosmic_signature_explanations(self):
    ...
    def _load_mafs(self):
        ...
        if os.path.isdir(self.maf):
            for filename in [n for n in os.listdir(self.maf) if n.endswith('maf')] or n.endswith('maf.gz'):
                file_path = '{}/{}'.format(self.maf, filename)
                self._load_maf(file_path, weight_for_multiple=True)
        else:
            self._load_maf(self.maf)

    def _load_maf(self, file_path, weight_for_multiple=False):
        """Load a MAF file's trinucleotide counts for each type of substitution"""
        self._status('Reading maf file: {}'.format(file_path))
        df = pd.read_csv(file_path, sep='\t', engine='python', skiprows=self.skip_rows, comment=self.comment)
        num_muts = len(df)
        self._status('- Total # muts: {}'.format(num_muts))
        ...
        self.num_samples += 1
        self._status('- Done, got a substitution dict of length: {}'.format(len(self.subs_dict)))

    def _get_snp_trinuc_context(self, df_row):
        ...
        if self.hg19_fasta:
            ...
            if len(ref_context) != 21:
                ...
                return None
            trinuc_context = self._standardize_trinuc(ref_context[9:12])
        return trinuc_context

    def _get_trinuc_context_from_fasta(self, df_row):
        ...
        return self.hg19_fasta[df_row.Chromosome][(position - 2):(position + 1)].seq

    @staticmethod
    def _standardize_subs(ref, alt):
        """
    ...
    @staticmethod
    def _standardize_trinuc(trinuc):
        """
    ...
    @staticmethod
    def _get_reconstructed_tumor_profile(signatures, w):
    ...
    def _get_error(self, tumor, signatures, w):
        """
        ...
        reconstructed_tumor_profile = self._get_reconstructed_tumor_profile(signatures, w)
        error = tumor - reconstructed_tumor_profile
        squared_error_sum = np.sum(error.dot(np.transpose(error)))
        return squared_error_sum

    def _update_weights(self, tumor, signatures, w, signatures_limit, ignorable_signature_indices=None):
        """
        ...
        error_old = self._get_error(tumor, signatures, w)
        ...
        for i in changeable_indices:
            ...
            def to_minimize(x):
                ...
                tmp[0, i] = x
                return self._get_error(tumor, signatures, w + tmp[0,])

            error_minimizer = minimize_scalar(to_minimize, bounds=(-w[i], 1), method="bounded").x
            v[i, i] = error_minimizer
            w_new = w + v[i]
            new_squared_errors[i] = self._get_error(tumor, signatures, w_new)
    ...
    def _seed_weights(self, tumor, signatures, ignorable_indices=None):
        """
        ...
        for i in range(num_sigs):
            if i not in ignorable_indices:
                ...
                tmp_weights[i] = 1
                error = self._get_error(tumor, signatures, tmp_weights)
                ss_errors[i] = error
